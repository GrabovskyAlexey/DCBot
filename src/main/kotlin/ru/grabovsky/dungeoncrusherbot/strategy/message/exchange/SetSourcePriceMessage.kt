...
@Component
class SetSourcePriceMessage(messageGenerateService: MessageGenerateService): AbstractSendMessage<PriceDto>(messageGenerateService) {
            ...
            (1..10).forEach { id ->
                add(
                    InlineMarkupDataDto(
                        ...
                        colPos = column,
                        text = id.toString(),
                        data = CallbackObject(StateCode.SET_SOURCE_PRICE, "SET_SOURCE_PRICE $id")
                    )
                )
                column++
                if (column > 5) {
