...
@Component
class RemoveExchangeRequestMessage(messageGenerateService: MessageGenerateService): AbstractSendMessage<ExchangeDetailDto>(messageGenerateService) {
    override fun inlineButtons(user: User, data: ExchangeDetailDto?, locale: Locale): List<InlineMarkupDataDto> {
        ...
        return buildList {
            data.requests.forEach { request ->
                add(
                    InlineMarkupDataDto(
                        ...
                        colPos = column,
                        text = "❌${request.pos}",
                        data = CallbackObject(StateCode.REMOVE_EXCHANGE_REQUEST, "REMOVE_REQUEST ${request.id}")
                    )
                )
                column++
                if (column > 5) {
            ...
            add(InlineMarkupDataDto(
                ...
                data = CallbackObject(StateCode.SET_SOURCE_PRICE, "BACK")
            ))
        }
    }
}