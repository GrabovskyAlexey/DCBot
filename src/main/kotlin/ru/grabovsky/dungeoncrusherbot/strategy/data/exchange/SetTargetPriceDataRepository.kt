...
@Repository
class SetTargetPriceDataRepository(
    ...
    override fun getData(user: TgUser): PriceDto {
        ...
        val resource = when {
            user.languageCode == "ru" && data.type == BUY_MAP -> "карт"
            user.languageCode == "ru" && data.type == SELL_MAP -> "пустот"
            data.type == BUY_MAP -> "maps"
            data.type == SELL_MAP -> "voids"
            else -> ""
        }
        return PriceDto(
            resource = resource
